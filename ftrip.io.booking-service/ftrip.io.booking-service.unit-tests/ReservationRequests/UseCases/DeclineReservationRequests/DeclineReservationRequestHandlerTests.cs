...
namespace ftrip.io.booking_service.unit_tests.ReservationRequests.UseCases.DeclineReservationRequests
{
    public class DeclineReservationRequestHandlerTests
    {
        ...
        public DeclineReservationRequestHandlerTests()
        {
            _handler = new DeclineReservationRequestHandler(
        ...
        {
            // Arrange
            var request = GetDeclineReservationRequest();
        ...
        public async Task Handle_Successful_ReturnsReservationRequest()
        {
            ...
            // Act
            var declinedReservationRequest = await _handler.Handle(request, CancellationToken.None);

            // Assert
            declinedReservationRequest.Should().NotBeNull();
            declinedReservationRequest.Status.Should().Be(ReservationRequestStatus.Declined);
            _unitOfWorkMock.Verify(uow => uow.Commit(It.IsAny<CancellationToken>()), Times.Once);
            _messagePublisherMock.Verify(mp => mp.Send<ReservationRequestDeclinedEvent, string>(It.IsAny<ReservationRequestDeclinedEvent>(), It.IsAny<CancellationToken>()), Times.Once);
        }

        private DeclineReservationRequest GetDeclineReservationRequest()
        {
